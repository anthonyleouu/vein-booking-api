...
function chauffeurPrice({ duration_hours, is_night, vehicleCfg, globalCfg }) {
  const hourlyRate = Number(vehicleCfg.chauffeur_hourly_rate || 0);
  ...
  let total = basePrice;
  const nightApplied = !!is_night;

  if (nightApplied) {
    if (nightType === "PERCENT") total = basePrice * (1 + nightValue / 100);
    else if (nightType === "FIXED") total = basePrice + nightValue;
  }
