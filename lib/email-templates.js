...
function buildTransferAdminEmail(booking) {
  ...
  const pickupShort = truncate(booking.pickup_address || "-", 25);
  const dropoffShort = truncate(booking.dropoff_address || "-", 25);
  const routeSummary = `${pickupShort} → ${dropoffShort}`;
  const customerLabel = [first, last].filter(Boolean).join(" ");

  const subject = `🚐 ${routeSummary} — ${date} ${time}, ${customerLabel}`;

  const waPhone = phoneForWA(booking.customer_phone);
  const waText = `Hi ${first || "there"}, this is Antonis from Selene Lux. Confirming your booking on ${date}.`;
  const waUrl = waPhone
  ...
  const tldr = `
    <div style="background:#2b2b2b;color:#f4f1ea;border-radius:10px;padding:16px 20px;margin-bottom:20px;font-size:17px;font-weight:700;text-align:center;line-height:1.5;">
      ${esc(booking.customer_name || "Guest")} booked ${esc(routeSummary)}<br>
      ${esc(date)} ${esc(time)} &mdash; &euro;${esc(String(booking.price_total_eur ?? "?"))}
    </div>
  `;
  ...
  const bookingBlock = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee7db;border-radius:12px;padding:0 16px;background:#fffdf9;margin-bottom:4px;">
      ...
      ${adminSummaryRow("Pickup Address", booking.pickup_address)}
      ${adminSummaryRow("Dropoff Address", booking.dropoff_address)}
  ...
  const notesBlock = `
    ...
    <ul style="margin:0 0 8px 0;padding-left:20px;font-size:15px;line-height:2.0;color:#2b2b2b;">
      <li>Add to calendar</li>
      <li>Confirm via WhatsApp</li>
      <li>Block buffer time</li>
    </ul>
  `;
...
===================================
${booking.customer_name || "Guest"} booked ${routeSummary}
${date} ${time} -- EUR ${booking.price_total_eur ?? "?"}
...
function buildTourAdminEmail(booking) {
  ...
  const subject = `🎯 ${tourName} — ${date} ${time}, ${customerLabel}`;
  ...
  const tldr = `
    <div style="background:#2b2b2b;color:#f4f1ea;border-radius:10px;padding:16px 20px;margin-bottom:20px;font-size:17px;font-weight:700;text-align:center;line-height:1.5;">
      ${esc(booking.customer_name || "Guest")} booked ${esc(tourName)}<br>
      ${esc(date)} ${esc(time)} &mdash; &euro;${esc(String(booking.price_total_eur ?? "?"))}
    </div>
  `;
  ...
  const bookingBlock = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee7db;border-radius:12px;padding:0 16px;background:#fffdf9;margin-bottom:4px;">
      ...
      ${adminSummaryRow("Pickup Date & Time", `${date} ${time}`)}
      ${adminSummaryRow(`Pickup (${pickupLabel})`, booking.pickup_address)}
      ${adminSummaryRow(`Dropoff (${dropoffLabel})`, booking.dropoff_address)}
...
================================
${booking.customer_name || "Guest"} booked ${tourName}
${date} ${time} -- EUR ${booking.price_total_eur ?? "?"}
...
Tour:       ${tourName}
Date/Time:  ${date} ${time}
Pickup (${pickupLabel}): ${booking.pickup_address || "-"}
Dropoff (${dropoffLabel}): ${booking.dropoff_address || "-"}
