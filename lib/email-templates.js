...
function baseEmailLayout({ previewText, header, bodyHtml, buttonLabel, buttonUrl, footerLines }) {
  const safePreview = esc(previewText);
  const safeHeader = esc(header);
  const safeButtonLabel = esc(buttonLabel);
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-radius:16px;overflow:hidden;">
            <tr>
              <td style="background:#2b2b2b;padding:28px 32px;text-align:center;">
            ...
            <tr>
              <td style="padding:24px 32px;background:#faf8f3;border-top:1px solid #e6e0d5;font-size:14px;line-height:1.6;color:#5c564d;">
                ${footerHtml}
              </td>
            </tr>
          </table>
...
function summaryRow(label, value) {
  return `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #eee7db;font-weight:600;color:#2b2b2b;vertical-align:top;width:42%;">
        ${esc(label)}
      </td>
      <td style="padding:10px 0;border-bottom:1px solid #eee7db;color:#2b2b2b;">
        ${esc(value || "-")}
      </td>
    </tr>
  `;
...
function buildTransferConfirmationEmail(booking) {
  ...
  const contactUrl = process.env.CONTACT_URL || "https://vip-athens-transfer.webflow.io/";
  const footerLines = [
    "<strong>Selene Lux</strong>",
    "Private Transfers & Tours",
    "contact@selenelux.com",
    "+306987253536",
  ...
  const bodyHtml = `
    <div style="font-size:16px;line-height:1.7;color:#2b2b2b;">
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px 0;border:1px solid #eee7db;border-radius:12px;padding:0 18px;background:#fffdf9;">
        ...
        ${summaryRow("Vehicle", booking.vehicle || "Mercedes V-Class")}
...
Vehicle: ${booking.vehicle || "Mercedes V-Class"}
