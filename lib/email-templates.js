...
function buildChauffeurConfirmationEmail(booking) {
  const customerName = booking.customer_name || "Guest";
  const subject = "Your chauffeur booking with Selene Lux is confirmed";
  const previewText = "Your chauffeur booking has been confirmed. Review your booking information below.";
  ...
  const durationLabel = booking.duration_hours != null ? `${booking.duration_hours} hours` : "-";
  ...
  const bodyHtml = `
    <div style="font-size:16px;line-height:1.7;color:#2b2b2b;">
      ...
      <p style="margin:0 0 24px 0;">
        Below you will find a summary of your chauffeur booking details. If you have any questions or need to update any information, please feel free to contact us.
      </p>
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px 0;border:1px solid #eee7db;border-radius:12px;padding:0 18px;background:#fffdf9;box-shadow:0 6px 18px rgba(0,0,0,0.05);">
        ...
        ${summaryRow("Booking ID", booking.booking_id)}
        ${summaryRow("Pickup Date & Time", formatDateTime(booking.start_time))}
        ${summaryRow("Pickup Location", normalizePlaceName(booking.pickup_address))}
  ...
  const text = `
Your chauffeur booking with Selene Lux is confirmed
...
Booking ID: ${booking.booking_id || "-"}
Pickup Date & Time: ${formatDateTime(booking.start_time)}
Pickup Location: ${normalizePlaceName(booking.pickup_address) || "-"}
...
// ─── Chauffeur admin email ────────────────────────────────────────────────────

function buildChauffeurAdminEmail(booking) {
  const { date, time } = formatDateParts(booking.start_time);
  const { first, last } = splitName(booking.customer_name);
  const customerLabel = [first, last].filter(Boolean).join(" ");
  const pickupShort = shortenLocation(booking.pickup_address);

  const subject = `🚗 Chauffeur — ${pickupShort}, ${date} ${time}, ${customerLabel}`.trim();
  ...
  const bookingBlock = `
    ${sectionHeader("Chauffeur Details")}
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee7db;border-radius:12px;padding:0 16px;background:#fffdf9;margin-bottom:4px;">
      ${adminSummaryRow("Pickup Date & Time", `${date} ${time}`)}
      ${adminSummaryRow("Pickup Address", normalizePlaceName(booking.pickup_address))}
      ${adminSummaryRow("Duration", durationLabel)}
      ${arrival ? adminSummaryRow("Arrival Reference", arrival) : ""}
      ${adminSummaryRow("Passengers", String(booking.passengers ?? "-"))}
      ${adminSummaryRow("Luggage", String(booking.luggage ?? "-"))}
      ${adminSummaryRow("Vehicle", "Mercedes Vito")}
  ...
  const html = baseEmailLayout({
    previewText: `${booking.customer_name || "Guest"} — ${pickupShort} — ${durationLabel} — ${date} ${time} — €${booking.price_total_eur ?? "?"}`,
    header: "Selene Lux — New Booking",
    bodyHtml,
    primaryLabel: null,
    primaryUrl: null,
  ...
  const text = `
NEW CHAUFFEUR BOOKING -- Selene Lux
====================================
...
Pickup:     ${date} ${time}
            ${normalizePlaceName(booking.pickup_address) || "-"}
Duration:   ${durationLabel}
${arrivalLine}Passengers: ${booking.passengers ?? "-"}
Luggage:    ${booking.luggage ?? "-"}
Vehicle:    Mercedes Vito
